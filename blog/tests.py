...
class TestView(TestCase):
    def setUp(self):
        ...
        self.post_002 = Post.objects.create(
            title="두 번째 포스트 입니다",
            content="It is not a world for only first?",
    ...
    def navbar_test(self, soup):
        ...
        about_me_btn = navbar.find(
            "a",
            text="About Me",
        )
        self.assertEqual(
            about_me_btn.attrs["href"],
            "/about_me/",
        )

    def category_card_test(self, soup):
        categories_card = soup.find(
            "div",
            id="categories-card",
        )
        self.assertIn(
    ...
    def test_post_list(self):
        ...
        self.assertIn(
            self.post_001.category.name,
            post_001_card.text,
        )

        post_002_card = main_area.find(
            "div",
            id="post-2",
        )
        self.assertIn(
        ...
        self.assertIn(
            self.post_002.category.name,
            post_002_card.text,
        )

        post_003_card = main_area.find(
            "div",
            id="post-3",
        )
        self.assertIn(
        ...
        self.assertIn(
            self.user_trump.username.upper(),
            main_area.text,
        )
        self.assertIn(
            self.user_obama.username.upper(),
            main_area.text,
        )
